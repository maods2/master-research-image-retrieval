...
def load_database(database_name):
    ...
    if database_name == 'Database 1':
        return np.random.rand(100, 128), [
            'Image ' + str(i) for i in range(100)
        ]
    elif database_name == 'Database 2':
        return np.random.rand(200, 128), [
            'Image ' + str(i) for i in range(200)
        ]
    else:
...
database_name = st.sidebar.selectbox(
    'Select Database', ['Database 1', 'Database 2']
)
top_k = st.sidebar.slider(
    'Number of Similar Images (K)', min_value=1, max_value=20, value=5
)
...
if uploaded_file:
    ...
    col1, col2 = st.columns(
        [1, 2]
    )  # Left column (smaller width), right column (larger width)
    ...
    if st.button('Run Query'):
        ...
        # Retrieve top-k similar images
        results = get_top_k(
            query_embedding, database_embeddings, database_labels, k=top_k
        )
        ...
        with col2:
            ...
            for idx, (label, similarity) in enumerate(results):
                st.image(
                    ...
                    caption=f'{label}: Similarity: {similarity:.4f}',

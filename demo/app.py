...
def get_top_k(query_embedding, db_embeddings, db_paths, db_labels, k=5):
    ...
    return [
        (db_paths[i], db_labels[i], similarities[i]) for i in top_k_indices
    ]
...
selected_db_name = st.sidebar.selectbox(
    'Selecione o banco de dados', list(available_dbs.keys())
)
top_k = st.sidebar.slider(
    'Número de imagens similares (K)', min_value=1, max_value=20, value=5
)
...
uploaded_file = st.file_uploader(
    'Envie uma imagem', type=['png', 'jpg', 'jpeg']
)
...
if uploaded_file:
    ...
    with col1:
        st.image(
            query_image, caption='Imagem de Consulta', use_container_width=True
        )

    if st.button('Buscar similares'):
        query_embedding = call_model(
            query_image, model, transform
        )  # substitua por modelo real
        results = get_top_k(
            query_embedding, db_embeddings, db_image_paths, db_labels, top_k
        )
        ...
        with col2:
            ...
            for path, label, similarity in results:
                if os.path.exists(path):
                    st.image(
                        path,
                        caption=f'{label} — Similaridade: {similarity:.4f}',

...
class TripletDataset(Dataset):
    def __init__(self, root_dir, transform=None, class_mapping=None):
        ...
        self.class_mapping = class_mapping or {class_name: idx for idx, class_name in enumerate(classes)}
        
        if any(class_name not in self.class_mapping for class_name in classes):
            raise ValueError("Some classes in the directory are not in the provided mapping.")
        ...
        for class_name in classes:
            ...
            images = [file for file in class_dir.rglob("*") if file.suffix.lower() in image_extensions]
            if len(images) < 2:
                raise ValueError(f"The class {class_name} has less than two images, which makes it impossible to form positive pairs.")
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        positive_path = random.choice([p for p in self.image_dict[anchor_class] if p != anchor_path])
        
        # Choose a negative from a different class
        negative_class = random.choice([cls for cls in self.image_dict.keys() if cls != anchor_class])
        ...
        positive = cv2.cvtColor(cv2.imread(str(positive_path)), cv2.COLOR_BGR2RGB)
        negative = cv2.cvtColor(cv2.imread(str(negative_path)), cv2.COLOR_BGR2RGB)
...
if __name__ == "__main__":
    ...
    for batch in data_loader:
        ...
        print("Anchor shape:", anchor.shape, "Positive shape:", positive.shape, "Negative shape:", negative.shape)
        break
...
def get_transforms(transform_config):
    ...
    if "gaussian_noise" in transform_config:
        transform_list.append(A.GaussNoise(
            var_limit=tuple(transform_config["gaussian_noise"]["var_limit"]),
    ...
    if "gaussian_blur" in transform_config:
        transform_list.append(A.GaussianBlur(
            blur_limit=tuple(transform_config["gaussian_blur"]["blur_limit"]),
    ...
    if "distortion" in transform_config:
        transform_list.append(A.OneOf([
            A.OpticalDistortion(p=transform_config["distortion"]["optical_distortion"]),
            A.GridDistortion(p=transform_config["distortion"]["grid_distortion"]),
            A.PiecewiseAffine(p=transform_config["distortion"]["piecewise_affine"]),
        ], p=transform_config["distortion"]["probability"]))
    
    if "shift_scale_rotate" in transform_config:
        transform_list.append(A.ShiftScaleRotate(
            shift_limit=transform_config["shift_scale_rotate"]["shift_limit"],
            scale_limit=transform_config["shift_scale_rotate"]["scale_limit"],
            rotate_limit=transform_config["shift_scale_rotate"]["rotate_limit"],
            interpolation=1,
            border_mode=0,
            value=(0, 0, 0),

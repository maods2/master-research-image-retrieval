...
class FewShotFolderDataset(StandardImageDataset):
    def __init__(self, root_dir, transform=None, class_mapping=None, config=None):
        ...
        """
        super(FewShotFolderDataset, self).__init__(root_dir, transform, class_mapping, config)
        
        self.root_dir = Path(root_dir)
        self.n_way = config['model'].get('n_way', 2) # number of classes per episode
        self.k_shot = config['model'].get('k_shot', 5) # support shots per class
        self.q_queries = config['model'].get('q_queries', 6) #  query shots per class
        self.transform = transform

        self.classes = [(i, cls) for i, cls in enumerate(list(self.class_mapping.keys()))]
    ...
    def __getitem__(self, idx):
        ...
        for i, cls in selected:
            imgs = random.sample(self.image_dict[self.class_mapping[cls]], self.k_shot + self.q_queries)

...
class Autoencoder(nn.Module):
    def __init__(
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(decoder_channels, 256, 3, stride=2, padding=1, output_padding=1),  # 8x8 -> 16x16
            nn.ReLU(),
            nn.ConvTranspose2d(256, 128, 3, stride=2, padding=1, output_padding=1),  # 16x16 -> 32x32
            nn.ReLU(),
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(32, 3, 4, stride=2, padding=1, output_padding=0),     # 128x128 -> 256x256
            nn.Tanh(),
        )
    def encode(self, x):

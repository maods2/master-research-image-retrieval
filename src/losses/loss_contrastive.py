...
class SupervisedContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, embeddings, labels):
        ...
        logits_mask = torch.ones_like(mask) - torch.eye(embeddings.size(0), device=mask.device)
...
class ProxyNCALoss(torch.nn.Module):
    ...
    def forward(self, embeddings, labels):
        ...
        loss = -torch.log(torch.exp(-dist[torch.arange(len(labels)), labels]).sum() / torch.exp(-dist).sum())
        return loss
...
class MultiSimilarityLoss(torch.nn.Module):
    ...
    def forward(self, embeddings, labels):
        ...
        pos_mask = torch.eq(labels.unsqueeze(0), labels.unsqueeze(1)).float() - torch.eye(len(labels), device=embeddings.device)
        neg_mask = torch.ne(labels.unsqueeze(0), labels.unsqueeze(1)).float()
        
        # Loss components
        pos_loss = (1.0 / self.alpha) * torch.log(1 + torch.sum(torch.exp(-self.alpha * (sim_matrix - self.base)) * pos_mask, dim=1))
        neg_loss = (1.0 / self.beta) * torch.log(1 + torch.sum(torch.exp(self.beta * (sim_matrix - self.base)) * neg_mask, dim=1))
...
class NPairLoss(torch.nn.Module):
    ...
    def forward(self, embeddings, labels):
        ...
        loss = -torch.log(torch.exp(sim_matrix * pos_mask).sum(dim=1) / torch.exp(sim_matrix * neg_mask).sum(dim=1))
        return loss.mean()
...
class FPN(nn.Module):
    ...
    def forward(self, features):
        ...
        for i in range(len(features) - 2, -1, -1):
            ...
            last_feat = nn.functional.interpolate(last_feat, size=feat.shape[-2:], mode='nearest') + feat
            outputs.insert(0, last_feat)
        
        # Apply convolution
        outputs = [fpn_conv(feat) for feat, fpn_conv in zip(outputs, self.fpn_convs)]
        return outputs
...
class AdvancedFeatureExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        fpn_features = self.attention(fpn_features.unsqueeze(1)).squeeze(1)  # Apply attention
...
class MultiTaskLoss(nn.Module):
    ...
    def forward(self, emb1, emb2, emb3, label):
        return self.contrastive_loss(emb1, emb2, label) + self.triplet_loss(emb1, emb2, emb3)

...
class HardNegativeMining:
    ...
    def get_hard_negatives(self, embeddings, labels):
        ...
        dists = torch.cdist(embeddings, embeddings)  # Compute pairwise distances
        hard_negatives = []
        for i in range(batch_size):
            negatives = dists[i][labels != labels[i]]  # Get different class samples
            if len(negatives) > 0:
...
class AdaptiveMultiTaskLoss(nn.Module):
    ...
    def forward(self, emb1, emb2, emb3, label):
        loss = self.contrastive_loss(emb1, emb2, label) + self.triplet_loss(emb1, emb2, emb3)
...
model = FineTunedFeatureExtractor(backbone="resnet50", embedding_dim=256).cuda()

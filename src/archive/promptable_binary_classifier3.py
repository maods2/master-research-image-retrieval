...
class PromptEncoder(nn.Module):
    ...
    def forward(self, pos_embeds, neg_embeds):
        ...
        pos_norm = F.normalize(
            pos_embeds, p=2, dim=-1
        )  # [B, num_tokens, embed_dim]
        neg_norm = F.normalize(
            neg_embeds, p=2, dim=-1
        )  # [B, num_tokens, embed_dim]

        # Compute cosine similarity matrix between all tokens.
        sim_matrix = torch.matmul(
            pos_norm, neg_norm.transpose(1, 2)
...
class PromptableBinaryClassifier(nn.Module):
    def __init__(
        ...
        self.prompt_encoder = PromptEncoder(
            in_dim=backbone_embed_dim, proj_dim=prompt_proj_dim
        )
    ...
    def forward(self, query_img, pos_prompts, neg_prompts):
        ...
        query_tokens = self.backbone(
            query_img
        )  # [B, num_patches, backbone_embed_dim]

        # Process prompts:
        # Flatten the prompt images: combine batch and prompt image dims.
        B_np, C, H, W = (
            pos_prompts.size(0) * pos_prompts.size(1),
            pos_prompts.size(2),
            pos_prompts.size(3),
            pos_prompts.size(4),
        )
        ...
        pos_tokens = self.backbone(
            pos_prompts_flat
        )  # [B*num_prompt_images, num_patches, backbone_embed_dim]
        neg_tokens = self.backbone(
            neg_prompts_flat
        )  # [B*num_prompt_images, num_patches, backbone_embed_dim]
        ...
        pos_prompt_embeds = pos_tokens.view(
            B, self.num_prompt_images * num_patches, -1
        )  # [B, num_tokens, backbone_embed_dim]
        neg_prompt_embeds = neg_tokens.view(
            B, self.num_prompt_images * num_patches, -1
        )  # [B, num_tokens, backbone_embed_dim]

        # Pass the full token sequences to the prompt encoder.
        prompt_stats = self.prompt_encoder(
            pos_prompt_embeds, neg_prompt_embeds
        )
        ...
        prompt_tokens = self.prompt_proj(
            prompt_tokens
        )  # [B, 4, backbone_embed_dim]

        # Concatenate query tokens with the projected prompt tokens.
        combined_tokens = torch.cat(
            [query_tokens, prompt_tokens], dim=1
        )  # [B, num_query_tokens+4, backbone_embed_dim]

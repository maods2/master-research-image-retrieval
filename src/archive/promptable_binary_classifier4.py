...
class PromptEncoder(nn.Module):
    def __init__(self, d_model=768, d_proj=768):
        ...
        self.similarity_proj = nn.Linear(
            4, d_proj
        )  # For 2x2 similarity matrix
    ...
    def forward(self, pos_embeds, neg_embeds):
        ...
        stats = torch.cat(
            [pos_mean, pos_std, neg_mean, neg_std], dim=1
        )  # (B, 784, d_proj)
...
class PromptableBinaryClassifier(nn.Module):
    ...
    def forward(self, query_img, pos_prompts, neg_prompts):
        ...
        pos_embeds = self.vit_backbone(
            pos_prompts.flatten(0, 1)
        )  # (B*2, 196, d_model)
        pos_embeds = pos_embeds.view(
            B, num_p, *pos_embeds.shape[-2:]
        )  # (B, 2, 196, d_model)
        ...
        final_tokens = torch.cat(
            [transformer_out, sim_proj.unsqueeze(1)], dim=1  # (B, 1, d_proj)
        )  # (B, 981, d_proj)
...
if __name__ == '__main__':
    ...
    model = PromptableBinaryClassifier(
        d_model=768, d_proj=768  # ViT model dimension  # Projection dimension
    )

...
class DINOLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output):
        ...
        teacher_probs = [
            (t - self.center) / self.teacher_temp for t in teacher_output
        ]
        ...
        for t in teacher_probs:
            for s in student_logits:
                loss = torch.sum(
                    -t * nn.functional.log_softmax(s, dim=-1), dim=-1
                ).mean()
                total_loss += loss
                n_loss_terms += 1
...
def train_dino(
    ...
    for epoch in range(epochs):
        ...
        for images, _ in loader:
            ...
            with torch.no_grad():
                teacher_outputs = [
                    teacher(v) for v in views[:2]
                ]  # only global views for teacher
            ...
            m = momentum_teacher * (
                1 - (1 - momentum_teacher) * (epoch / epochs)
            )
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='DINOv2 Self-Supervised Training'
    )
    parser.add_argument(
        'data_dir', type=str, help='Path to ImageNet or image folder'
    )

...
class ViTEncoder(EncoderBase):
    def __init__(self, latent_dim: int, image_size: int = 224, patch_size: int = 16, 
                 dim: int = 512, depth: int = 6, heads: int = 8, in_channels: int = 3):
        super().__init__(latent_dim)
...
class VQVAE(nn.Module):
    def __init__(self, encoder: EncoderBase, num_embeddings: int = 512, 
    ...
    def forward(self, x):
        ...
        d = torch.sum(z ** 2, dim=1, keepdim=True) + \
            ...
            2 * torch.matmul(z.view(-1, self.embedding_dim), 
        ...
        z_q = self.embedding(min_encoding_indices).view(-1, self.embedding_dim, 1, 1)
...
class VQVAELoss(nn.Module):
    ...
    def forward(self, x_recon, x, z, z_q):
        ...
        # VQ loss
        vq_loss = self.mse_loss(z_q.detach(), z) + \

...
class MaskedAutoencoder(nn.Module):
    def __init__(self):
        ...
        self.decoder_pos_embed = nn.Embedding(
            self.num_patches, self.decoder_embed
        )
        self.decoder_head = nn.Linear(
            self.decoder_embed, self.patch_size**2 * 3
        )
    ...
    def forward_decoder(self, x, ids_shuffle):
        ...
        mask_tokens = self.mask_token.repeat(
            B, ids_restore.shape[1] - x.shape[1], 1
        )
        x_ = torch.cat([x, mask_tokens], dim=1)
        x = torch.gather(
            x_,
            dim=1,
            index=ids_restore.unsqueeze(-1).repeat(1, 1, self.decoder_embed),
        )
